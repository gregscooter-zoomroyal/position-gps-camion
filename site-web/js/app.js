(() => {
  const STORAGE_KEY = "encan-studio-v1";
  ...
  function vehicles() {
    return [...(window.ENCAN.vehicles || []), ...state.custom.vehicles];
  }

  function mediaItems() {
    return [...(window.ENCAN.media || []), ...state.custom.media];
  }
  ...
  function renderEvents(sel) {
    const root = $(sel);
    if (!root) return;
    root.innerHTML = ENCAN.events.map(e => `
  ...
  function bind() {
    ...
    $("#hero-play").addEventListener("click", () => {
      openLightbox(ENCAN.heroVideo, "L'Encan — bande-annonce");
    });
  ...
  function bootHero() {
    ...
    if (poster) poster.src = ENCAN.heroPoster;
    video.poster = ENCAN.heroPoster;
    video.src = ENCAN.heroVideo;
    ...
    $("#stats").innerHTML = ENCAN.stats.map(s => `<div class="stat"><b>${esc(s.value)}</b><span>${esc(s.label)}</span></div>`).join("");
  }
